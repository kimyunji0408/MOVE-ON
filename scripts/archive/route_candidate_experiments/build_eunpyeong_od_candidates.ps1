...
function Extract-Route($Payload, $RouteId, $Origin, $Destination, $ThroughStation) {
    ...
    for ($i = 0; $i -lt $paths.Count; $i++) {
        $path = $paths[$i]
        if ($i -eq 0) {
            $stations.Add([string]$path.dptreStn.stnNm) | Out-Null
            $stationLines.Add([string]$path.dptreStn.lineNm) | Out-Null
        }
        $stations.Add([string]$path.arvlStn.stnNm) | Out-Null
        $stationLines.Add([string]$path.arvlStn.lineNm) | Out-Null
    ...
    return [pscustomobject][ordered]@{
        ...
        station_line_sequence = @($stationLines)
        line_sequence = @($lines)
...
function Route-Signature($Route) {
    return (($Route.station_sequence -join ">") + "|" + ($Route.station_line_sequence -join ">"))
}
...
function Get-RouteAccessibility($Route, $AccessIndex) {
    ...
    for ($i = 0; $i -lt $Route.station_sequence.Count; $i++) {
        ...
        $line = if ($Route.station_line_sequence.Count -gt $i) { $Route.station_line_sequence[$i] } elseif ($Route.line_sequence.Count -eq 1) { $Route.line_sequence[0] } else { $Route.line_sequence[[Math]::Min($i, $Route.line_sequence.Count - 1)] }
        $key = "$station|$line"
...
$flood.features = @($flood.features | Where-Object { ([string]$_.properties.ADM_CD).StartsWith("11380") })
$idx = 0
...
for ($oi = 0; $oi -lt $stationNames.Count; $oi++) {
    for ($di = 0; $di -lt $stationNames.Count; $di++) {
        ...
        $baselineRecord = [pscustomobject][ordered]@{
            ...
            station_sequence = $baselineRoute.station_sequence; station_line_sequence = $baselineRoute.station_line_sequence; line_sequence = $baselineRoute.line_sequence
        ...
        foreach ($through in $stationNames) {
            ...
            $altRecord = [pscustomobject][ordered]@{
                ...
                station_sequence = $altRoute.station_sequence; station_line_sequence = $altRoute.station_line_sequence; line_sequence = $altRoute.line_sequence
...
$csvRows = @($candidateRows | ForEach-Object {
    [pscustomobject][ordered]@{
        ...
        station_sequence = ($_.station_sequence -join ">"); station_line_sequence = ($_.station_line_sequence -join ">"); line_sequence = ($_.line_sequence -join ">")
